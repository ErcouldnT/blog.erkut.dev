<script lang="ts">
	...
	// Utils
	import infoDate from '$lib/utils/infoDate';
	...
	let readingMinute: string;

	post.then(({ post }) => {
		isPost = post ? true : false;

		if (post) {
			editable = user?.id === post.author; // or admin
			readingMinute = readingTime(post.content ?? '');
		}
	});
</script>
